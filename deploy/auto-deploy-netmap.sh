...
# SELF-UPDATE HAZARD -- why everything below lives in main().
#
# This script deploys the repository it is stored in, so `git reset --hard`
# rewrites THIS FILE while bash is executing it. Bash does not slurp a script
# into memory: it reads and parses incrementally, tracking a byte offset. If
# the file grows or shrinks underneath a running shell, execution resumes at
# the wrong offset -- mid-line, mid-word -- and bash runs whatever fragment it
# finds there. On a script that stops containers and does `git reset --hard`
# that is not a theoretical concern.
#
# Wrapping the body in a function and calling it on the last line means bash
# parses the entire body before a single command of it runs, so a rewrite
# mid-deploy cannot change what this invocation does. The next run picks up
# the new version, which is exactly the intended behaviour.
set -uo pipefail
